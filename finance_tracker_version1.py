...
class ExpenseTracker:
    ...
    def view_income(self, month=None, year=None):
        ...
        total = 0
        for inc in sorted(filtered, key=lambda x: x['date'], reverse=True):
    ...
    def delete_income(self, index):
        ...
        if 0 <= index < len(self.income):
            deleted = self.income.pop(index)
            self.save_data()
            print(f"✓ Deleted income: {deleted}")
        else:
            print("Invalid index")
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            print("\nIncome Sources:", ", ".join(tracker.income_sources))
            ...
            del_choice = input("Select: ").strip()
            
            if del_choice == '1':
                tracker.view_expenses()
                index = input("Enter expense index to delete: ").strip()
                try:
                ...
                tracker.view_income()
                index = input("Enter income index to delete: ").strip()
                try:
